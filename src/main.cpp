...
#if defined(NDEBUG)
#error "CleanSolutions cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "CleanSolutions Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 40000 * COIN;
		
	int64_t nSubsidy;
	
	if( nHeight > 1 && nHeight <= 12000 ) {
	        nSubsidy = 1 * COIN;
	} else if( nHeight > 12000 && nHeight <= 140000 ) {
	        nSubsidy = 14 * COIN;
	} else {
		nSubsidy = 9 * COIN;
	}
...
{
    RenameThread("cleansolutions-scriptch");
