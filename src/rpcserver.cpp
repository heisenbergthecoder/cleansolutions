...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop CleanSolutions server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "CleanSolutions server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* CSN features */
        {"cleansolutions", "masternode", &masternode, true, true, false},
        {"cleansolutions", "masternodelist", &masternodelist, true, true, false},
        {"cleansolutions", "mnbudget", &mnbudget, true, true, false},
        {"cleansolutions", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"cleansolutions", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"cleansolutions", "mnsync", &mnsync, true, true, false},
        {"cleansolutions", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"cleansolutions", "Darksend", &Darksend, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use cleansolutionsd, or the -server option to cleansolutions-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
                                               ...
                                               "for example: alertnotify=echo %%s | mail -s \"CleanSolutions Alert\" admin@foo.com\n"),
...
{
    return "> cleansolutions-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:8422/\n";
}
